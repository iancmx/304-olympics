...
const allAthletes = async (req, res) => {
  db.query("SELECT A.athlete_id AS id, P.name, A.age, A.sex, A.weight, A.height, P.country, A.gold_medal_count, A.silver_medal_count, A.bronze_medal_count FROM athlete A, participant P WHERE A.participant_id = P.participant_id", (err, result, fields) => {
    if (err) throw err;
    res.json(result);
  });
};
